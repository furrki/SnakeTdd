...
class SnakeInitializationTests: XCTestCase {
    ...
    func testInitTail() {
        ...
        XCTAssertEqual(snakeDown.tail, [CGPoint(x: 10, y: 7), CGPoint(x: 10, y: 8), CGPoint(x: 10, y: 9)])
        
        let snakeUp: Snake = Snake(headPos: CGPoint(x: 10, y: 10), direction: .up, initialSize: 4)
        XCTAssertEqual(snakeUp.tail, [CGPoint(x: 10, y: 13), CGPoint(x: 10, y: 12), CGPoint(x: 10, y: 11)])
        
        let snakeLeft: Snake = Snake(headPos: CGPoint(x: 10, y: 10), direction: .left, initialSize: 4)
        XCTAssertEqual(snakeLeft.tail, [CGPoint(x: 13, y: 10), CGPoint(x: 12, y: 10), CGPoint(x: 11, y: 10)])
        
        let snakeRight: Snake = Snake(headPos: CGPoint(x: 10, y: 10), direction: .right, initialSize: 4)
        XCTAssertEqual(snakeRight.tail, [CGPoint(x: 7, y: 10), CGPoint(x: 8, y: 10), CGPoint(x: 9, y: 10)])
    }
